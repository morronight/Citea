...
function go()
{
	...
	xhr.send("lieuxD="+idGareDep+"&lieuxA="+idGareArr+"&methode=ajax&action=afficher&heuresD="+heureD+"&minutesD="+minutesD);
...
function donneesOffline()
{
	...
	{
		if((mesLignes[i].getAttribute("data-depart")==idGareDep) && (mesLignes[i].getAttribute("data-arrivee")==idGareArr) && (mesLignes[i].getAttribute("data-heure") > (timesec - 300)) && (compteur < 2))
		{
...
function DisplayCansii()
{
   ...
   function afficher(ma_balise,etat)
	...
	function aff_Valence_Romans()
		{
		if (document.getElementById('ajout').selected == true)
		{	
		document.getElementById('valence2').style.display = 'none';
		document.getElementById('ajout2').style.display = 'block';
		}
		else
		{
		document.getElementById('ajout2').style.display = 'none';
		if (document.getElementById('valence_1').selected == true || 
				document.getElementById('valence_2').selected == true ||
				document.getElementById('valence_3').selected == true ||
				document.getElementById('valence_4').selected == true)
			{	
			document.getElementById('valence2').style.display = 'block';
			}
		}
		}
	...
	IdVille = option[option.selectedIndex].getAttribute("data-IdGare");
	...
	url += '?IdVille='+IdVille+'&action=ajoutselect&methode=ajax';
	if (heureMin != '')
	...
	formData.append("action", "ajoutselect");
	formData.append("methode", "ajax");*/
	...
	{ 
		if (etat == 'afficher')
			{document.getElementById(balise).style.display = 'inline';}
